...
# path can be changed
analysis_type = "default_christian" #["default_type1", "default_type2", "default_type3", "customtype1", "default_type4_random", default_christian]
...
def concat_new_data(data, exp_id, json_file, csv_file, analysis_type = "default"):
    ...
    data.loc[data["experimentID"] == exp_id, f"madap_arr_fitted_log_conductivity_{analysis_type} [ln(S/cm)]"] = csv_file["log_conductivity_fit [ln(S/cm)]"]
...
processed_data.to_csv(os.path.join(os.getcwd(),fr"data/arr_data/processed_data_arr_and_impedance_{analysis_type}.csv"), sep=";", index=True, mode='w+')
