...
path_to_json = fr"C:\Repositories\MADAP\electrolyte_figures\arrhenius_{analysis_type}\data"
...
def concat_new_data(data, exp_id, json_file, csv_file, analysis_type = "default"):
    # 1. activation
    data.loc[data["experimentID"] == exp_id, f"madap_arr_activation_energy [mJ/mol]"] = json_file["activation [mJ/mol]"]
    # 2. cell constant
    data.loc[data["experimentID"] == exp_id, f"madap_arr_activation_constant"] = json_file["arr_constant [S.cm⁻¹]"]
    # 3. r2 score
    data.loc[data["experimentID"] == exp_id, f"madap_arr_activation_r2_score"] = json_file["R2_score"]
    # 4. mse score
    data.loc[data["experimentID"] == exp_id, f"madap_arr_activation_mse"] = json_file["MSE"]
    # 5. log conductivity
    ...
    data.loc[data["experimentID"] == exp_id, f"madap_arr_fitted_log_conductivity [ln(S/cm)]"] = csv_file["log_conductivity_fit [ln(S/cm)]"].values
